...
from coingro_controller.constants import (BOT_NAME_ADJECTIVES, DEFAULT_EXCHANGE,
...
class Controller(LoggingMixin):
    ...
    def create_bot(self,
                   ...
                   update: bool = False,
                   env_vars: Dict[str, Any] = {}) -> Tuple[str, str]:
        if not bot_id:
            uid = uuid.uuid4().hex
            bot_id = f'bot-{uid}'
        ...
        if not is_deleted:
            ...
            if not bot:
                ...
                if is_strategy:
                    ...
                    bot.stake_currency = self.config.get('default_strategy_stake_currency',
                                                         DEFAULT_STAKE_CURRENCY)
            ...
            bot.api_url = f"http://{bot_id}/{self.config['cg_api_router_prefix']}" \
                          if 'cg_api_router_prefix' in self.config else f'http://{bot_id}'
            if update:
    ...
    def check_strategies(self) -> None:
        ...
        for obj in strategy_objects:
            ...
            if name not in Strategy.strategy_names():
                bot_id, _ = self.create_bot(bot_id=name, bot_name=name,
                                            is_strategy=True, env_vars=env)
                bot = Bot.bot_by_id(bot_id)
                strategy = Strategy(bot_id=bot.id if bot else None,  # Should not be none
