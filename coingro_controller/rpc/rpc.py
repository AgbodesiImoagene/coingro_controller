...
class RPC:
    ...
    def __init__(self, coingro_controller) -> None:
        ...
        :return: None
        """
        self._coingro_controller = coingro_controller
        self._client = coingro_controller.coingro_client
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.open_order_id:
                    order = self._coingro_controller.exchange.fetch_order(trade.open_order_id, trade.pair)
                # calculate profit and send message to user
                if trade.is_open:
                    try:
                        current_rate = self._coingro_controller.exchange.get_rate(
                ...
                if len(trade.select_filled_orders(trade.entry_side)) > 0:
                    ...
                    if self._fiat_converter:
                        current_profit_fiat = self._fiat_converter.convert_amount(
                            current_profit_abs,
                            self._coingro_controller.config['stake_currency'],
                            self._coingro_controller.config['fiat_display_currency']
                        )
                else:
    ...
    def _rpc_timeunit_profit(
        ...
        daily_stake = self._coingro_controller.wallets.get_total_stake_amount()
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._coingro_controller.wallets.get_starting_balance()
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> Dict:
        ...
        total = 0.0
        try:
            tickers = self._coingro_controller.exchange.get_tickers(cached=True)
        except (ExchangeError):
            raise RPCException('Error getting current tickers.')

        self._coingro_controller.wallets.update(require_update=False)
        starting_capital = self._coingro_controller.wallets.get_starting_balance()
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._coingro_controller.wallets.get_all_balances().items():
            ...
            if coin == stake_currency:
                ...
                try:
                    pair = self._coingro_controller.exchange.get_valid_pair_combination(coin, stake_currency)
        ...
        for symbol, position in self._coingro_controller.wallets.get_all_positions().items():
        ...
        return {
            ...
            'note': 'Simulated balances' if self._coingro_controller.config['dry_run'] else ''
        }

    def _rpc_start(self) -> Dict[str, str]:
        """ Handler for start """
        if self._coingro_controller.state == State.RUNNING:
            return {'status': 'already running'}

        self._coingro_controller.state = State.RUNNING
        return {'status': 'starting trader ...'}

    def _rpc_stop(self) -> Dict[str, str]:
        """ Handler for stop """
        if self._coingro_controller.state == State.RUNNING:
            self._coingro_controller.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> Dict[str, str]:
        ...
        self._coingro_controller.state = State.RELOAD_CONFIG
    ...
    def _rpc_stopbuy(self) -> Dict[str, str]:
        ...
        if self._coingro_controller.state == State.RUNNING:
            ...
            self._coingro_controller.config['max_open_trades'] = 0
    ...
    def _rpc_force_exit(self, trade_id: str, ordertype: Optional[str] = None) -> Dict[str, str]:
        ...
        def _exec_force_exit(trade: Trade) -> None:
            ...
            if trade.open_order_id:
                ...
                if order['side'] == trade.entry_side:
                    fully_canceled = self._coingro_controller.handle_cancel_enter(
                        trade, order, CANCEL_REASON['FORCE_EXIT'])

                if order['side'] == trade.exit_side:
                    # Cancel order - so it is placed anew with a fresh price.
                    self._coingro_controller.handle_cancel_exit(trade, order, CANCEL_REASON['FORCE_EXIT'])
            ...
            if not fully_canceled:
                ...
                order_type = ordertype or self._coingro_controller.strategy.order_types.get(
                    "force_exit", self._coingro_controller.strategy.order_types["exit"])

                self._coingro_controller.execute_trade_exit(
        ...
        with self._coingro_controller._exit_lock:
            if trade_id == 'all':
                # Execute sell for all open orders
                for trade in Trade.get_open_trades():
                    _exec_force_exit(trade)
                Trade.commit()
                self._coingro_controller.wallets.update()
    ...
    def _rpc_force_entry(self, pair: str, price: Optional[float], *,
        ...
        if not self._coingro_controller.config.get('force_entry_enable', False):
            raise RPCException('Force_entry not enabled.')

        if self._coingro_controller.state != State.RUNNING:
            raise RPCException('trader is not running')

        if order_side == SignalDirection.SHORT and self._coingro_controller.trading_mode == TradingMode.SPOT:
            raise RPCException("Can't go short on Spot markets.")

        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._coingro_controller.config.get('stake_currency')
        if not self._coingro_controller.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
        ...
        if trade:
            ...
            if not self._coingro_controller.strategy.position_adjustment_enable:
                raise RPCException(f'position for {pair} already open - id: {trade.id}')

        if not stake_amount:
            # gen stake amount
            stake_amount = self._coingro_controller.wallets.get_trade_stake_amount(pair)

        # execute buy
        if not order_type:
            order_type = self._coingro_controller.strategy.order_types.get(
                'force_entry', self._coingro_controller.strategy.order_types['entry'])
        if self._coingro_controller.execute_entry(pair, stake_amount, price,
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._coingro_controller._exit_lock:
            ...
            if trade.open_order_id:
                try:
                    self._coingro_controller.exchange.cancel_order(trade.open_order_id, trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass

            # cancel stoploss on exchange ...
            if (self._coingro_controller.strategy.order_types.get('stoploss_on_exchange')
                    and trade.stoploss_order_id):
                try:
                    self._coingro_controller.exchange.cancel_stoploss_order(trade.stoploss_order_id,
                                                                 trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass
    ...
    def _rpc_count(self) -> Dict[str, float]:
        ...
        return {
            'current': len(trades),
            'max': (int(self._coingro_controller.config['max_open_trades'])
                    if self._coingro_controller.config['max_open_trades'] != float('inf') else -1),
    ...
    def _rpc_whitelist(self) -> Dict:
        ...
        res = {'method': self._coingro_controller.pairlists.name_list,
               'length': len(self._coingro_controller.active_pair_whitelist),
               'whitelist': self._coingro_controller.active_pair_whitelist
               }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: List[str]) -> Dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._coingro_controller.pairlists.blacklist:
                self._coingro_controller.pairlists.blacklist.remove(pair)
            else:
                errors[pair] = {
    ...
    def _rpc_blacklist(self, add: List[str] = None) -> Dict:
        ...
        if add:
            for pair in add:
                if pair not in self._coingro_controller.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], self._coingro_controller.exchange.get_markets().keys())
                        self._coingro_controller.pairlists.blacklist.append(pair)
        ...
        res = {'method': self._coingro_controller.pairlists.name_list,
               'length': len(self._coingro_controller.pairlists.blacklist),
               'blacklist': self._coingro_controller.pairlists.blacklist,
               'blacklist_expanded': self._coingro_controller.pairlists.expanded_blacklist,
               'errors': errors,
               }
        return res
    ...
    def _rpc_edge(self) -> List[Dict[str, Any]]:
        ...
        if not self._coingro_controller.edge:
            raise RPCException('Edge is not enabled.')
        return self._coingro_controller.edge.accepted_pairs()
    ...
    def _rpc_analysed_dataframe(self, pair: str, timeframe: str,
        ...
        _data, last_analyzed = self._coingro_controller.dataprovider.get_analyzed_dataframe(
            pair, timeframe)
        _data = _data.copy()
        if limit:
            _data = _data.iloc[-limit:]
        return self._convert_dataframe_to_dict(self._coingro_controller.config['strategy'],
    ...
    def _rpc_plot_config(self) -> Dict[str, Any]:
        if (self._coingro_controller.strategy.plot_config and
                'subplots' not in self._coingro_controller.strategy.plot_config):
            self._coingro_controller.strategy.plot_config['subplots'] = {}
        return self._coingro_controller.strategy.plot_config
    ...
    def _state(self) -> Dict[str, str]:
        return {
            ...
            'message': self._coingro_controller.message,
        }
