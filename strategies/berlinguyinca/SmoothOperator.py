...
class SmoothOperator(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["mfi_rsi_cci_smooth"] = (
            dataframe["rsi_smooth"] * 1.125
            + dataframe["mfi_smooth"] * 1.125
            + dataframe["cci_smooth"]
        ) / 3

        dataframe["mfi_rsi_cci_smooth"] = ta.TEMA(
            dataframe, timeperiod=21, price="mfi_rsi_cci_smooth"
        )

        # playgound
        dataframe["candle_size"] = (
            (dataframe["close"] - dataframe["open"]) * (dataframe["close"] - dataframe["open"]) / 2
        )
